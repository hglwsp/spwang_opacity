...
############################################
DIM_S = 6 # dimension of system
DIM_C = 7 # dimension of controller input
...
# DATA_EXP_I = np.array([4,4,4,4,4,4])
...
# BLOCK_EXP_I = np.array([3,3,3,3,3,3])
#     # 0 <= BATCH_EXP <= DATA_EXP
# BLOCK_LEN_I = np.power(2, BLOCK_EXP_I)
#     # number of batches for each dimension

# DATA_EXP_U = np.array([4,4,4,4,4,4]) # for sampling from initial; length = prob.DIM
# DATA_LEN_U = np.power(2, DATA_EXP_U) # the number of samples for each dimension of domain
# BLOCK_EXP_U = np.array([3,3,3,3,3,3]) # 0 <= BATCH_EXP <= DATA_EXP
# BLOCK_LEN_U = np.power(2, BLOCK_EXP_U) # number of batches for each dimension
DATA_LEN_I = np.array([12,12,12,12,12,12])
BLOCK_LEN_I = np.array([6,6,6,6,6,6])
DATA_LEN_U = np.array([12,12,12,12,12,12])
BLOCK_LEN_U = np.array([6,6,6,6,6,6])
DATA_LEN_D = np.array([12,12,12,12,12,12])
BLOCK_LEN_D = np.array([6,6,6,6,6,6])
# DATA_EXP_D = np.array([4,4,4,4,4,4]) # for sampling from initial; length = prob.DIM
# DATA_LEN_D = np.power(2, DATA_EXP_D) # the number of samples for each dimension of domain
# BLOCK_EXP_D = np.array([3,3,3,3,3,3]) # 0 <= BATCH_EXP <= DATA_EXP
