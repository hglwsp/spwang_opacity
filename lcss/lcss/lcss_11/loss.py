...
def calc_loss(barr_nn, ctrl_nn, input_init, input_unsafe, input_domain, epoch):
    ...
    loss_diff = torch.relu(output_fx - output_x + 0.001)
    ...
    p = input_domain[:, 4].view(-1, 1)
    q = input_domain[:, 5].view(-1, 1)
    e1 = e[:,0].view(-1, 1)
    input_u = torch.cat((a,b,c, d,p,q,e1), dim=1)
    output_u = ctrl_nn(input_u)
    loss_u = torch.relu(-(output_u) - 0.5) + torch.relu((output_u) - 0.5)
