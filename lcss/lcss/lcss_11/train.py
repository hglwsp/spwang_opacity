    ...
    while num_restart < 5:
        ...
        for epoch in range(superp.EPOCHS):  # train for a number of epochs
            ...
            for batch_index in range(NUM_BATCHES[3]):
                ...
                curr_init,curr_unsafe,curr_diff,curr_u,curr_batch_loss, curr_batch_gradient = loss.calc_loss(barr_nn, ctrl_nn, batch_init, batch_unsafe,
                                                                      batch_domain, epoch)
                ...
                if superp.VERBOSE == 1:
                    print("restart: %-2s" % num_restart, "epoch: %-3s" % epoch, "batch: %-5s" % batch_index, \
                          ...
                          # "boundary_gradient: %-5s" % superp.CURR_MAX_GRAD,\
