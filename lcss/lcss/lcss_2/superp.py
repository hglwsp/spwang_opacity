...
BLOCK_EXP_I = np.array([8,8,0,0])
    # 0 <= BATCH_EXP <= DATA_EXP
BLOCK_LEN_I = np.power(2, BLOCK_EXP_I) 
    # number of batches for each dimension

DATA_EXP_U = np.array([5,5,5,5]) # for sampling from initial; length = prob.DIM
...
DATA_EXP_D = np.array([5,5,5,5]) # for sampling from initial; length = prob.DIM
