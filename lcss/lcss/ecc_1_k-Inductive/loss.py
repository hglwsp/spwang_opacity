...
k-Inductive Control Barrier Certificates!
"""
############################################
...
############################################
def calc_loss(barr_nn, ctrl_nn,ctrl_nn2, input_init, input_unsafe, input_domain,epoch):
    ...
    f_domain = prob.vector_field(input_domain, ctrl_nn, ctrl_nn2)
    output_fx = barr_nn(f_domain)
    loss_diff1 = torch.zeros([len(output_x), 1])
    for i in range(len(output_x)):
        if output_x[i] <= 0:
            loss_diff1[i] = torch.relu(output_fx[i] + 0.001)

    # loss_diff1 = torch.relu(output_fx + 0.001)
    ...
    output_u2 = ctrl_nn2(input_u2)
    output_u3 = ctrl_nn(input_u3)
    output_u4 = ctrl_nn2(input_u4)
    loss_u = torch.relu(-(output_u1)) + torch.relu((output_u1) - 1000) \
             + torch.relu(-(output_u2)) + torch.relu((output_u2) - 1000)\
             + torch.relu(-(output_u3)) + torch.relu((output_u3) - 1000)\
             + torch.relu(-(output_u4)) + torch.relu((output_u4) - 1000)
