...
def calc_loss(barr_nn, ctrl_nn, input_init, input_unsafe1, input_unsafe2,input_domain, epoch):
    ...
    loss_unsafe_1 = torch.relu((- output_unsafe1) + 1.001 + 0.001)  # tolerance

    # compute loss of unsafe1
    output_unsafe2 = barr_nn(input_unsafe2)
    loss_unsafe_2 = torch.relu((- output_unsafe2) + 1.001  + 0.001)  # tolerance
    ...
    loss_u = torch.relu(-(output_u) - 3) + torch.relu((output_u) - 3)
