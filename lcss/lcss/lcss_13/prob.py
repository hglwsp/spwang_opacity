...
def cons_init(x):
    # return x[:, 0] == x[:, 0]
    inner_box = ((x[:,1] - x[:,0]) * (x[:,1] - x[:,0]) <= 0.8*0.8) \
                & (0 <= x[:, 0]) & (x[:, 0] <= 1) \
                & (0 <= x[:, 1]) & (x[:, 1] <= 1) \
                & (-pi/16 <= x[:, 2]) & (x[:, 2] <= pi/16) \
                & (-pi/16 <= x[:, 3]) & (x[:, 3] <= pi/16)
...
def cons_unsafe(x):
    inner_box = ((x[:,1] - x[:,0]) * (x[:,1] - x[:,0]) > 1.5*1.5) \
              & (-6 <= x[:,0]) &  (x[:,0]<= 6) \
              & (-6 <= x[:,1] ) & (x[:,1]<= 6)\
              & (-0.7*pi <= x[:,2]) & (x[:,2] <= 0.7*pi)\
               & (-0.7*pi <= x[:,3]) & (x[:,3] <= 0.7*pi)
    return inner_box
