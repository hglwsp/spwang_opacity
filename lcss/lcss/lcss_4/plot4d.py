...
import matplotlib
from verify.opacityy import prob, superp
...
############################################
INIT = [prob.INIT[0], prob.INIT[1]]
UNSAFE = [prob.UNSAFE[0], prob.UNSAFE[1]]
DOMAIN = [prob.DOMAIN[0], prob.DOMAIN[1]]
def plot_sys_4d(barr_nn, ctrl_nn):
    ...
    def gen_plot_data(region, len_sample):
        grid_sample = [torch.linspace(region[i][0], region[i][1],int(len_sample[i])) for i in
                       range(superp.DIM_S - 2)]  # gridding each dimension
    ...
    def plot_boundary():  # barrier boundary: contour plotting
        barrier_plot_nn_input = gen_plot_data(DOMAIN, superp.PLOT_LEN_B)
        # apply the nn model but do not require gradient
        with torch.no_grad():
            # barrier_plot_nn_output = barr_nn(barrier_plot_nn_input).reshape(
            #     superp.PLOT_LEN_B[3],superp.PLOT_LEN_B[2],superp.PLOT_LEN_B[1],
            #                                                                 superp.PLOT_LEN_B[0])  # y_size * x_size
            barrier_plot_nn_output = barr_nn(barrier_plot_nn_input).reshape(superp.PLOT_LEN_B[1],
                                                                            superp.PLOT_LEN_B[0])  # y_size * x_size

        plot_Z = barrier_plot_nn_output.numpy()
        plot_sample_x = np.linspace(DOMAIN[0][0], DOMAIN[0][1], superp.PLOT_LEN_B[0])
        plot_sample_y = np.linspace(DOMAIN[1][0], DOMAIN[1][1], superp.PLOT_LEN_B[1])
    ...
    def plot_scatter():  # scatterring sample points
        scatter_plot_nn_input = gen_plot_data(DOMAIN, superp.PLOT_LEN_P)
    ...
    def plot_vector_field():  # vector field
        vector_plot_nn_input = gen_plot_data(DOMAIN, superp.PLOT_LEN_V)
        with torch.no_grad():
        ...
        vector_plot = plt.quiver(vector_x_positions, vector_y_positions, vector_x_values, vector_y_values, \
                                 color='black', width=0.001, headwidth=6, headlength=6, headaxislength=3, angles='xy',
                                 scale_units='xy', scale=superp.PLOT_VEC_SCALE)
        return vector_plot
    ...
    def plot_init(init_range, init_shape):
        if init_shape == 1:  # rectangle
            init = matplotlib.patches.Rectangle((init_range[0][0], init_range[1][0]), \
                                                ...
                                                init_range[1][1] - init_range[1][0], facecolor='green')
        if init_shape == 2:  # circle
            init = matplotlib.patches.Circle(((init_range[0][1] + init_range[0][0]) / 2.0, \
                                              (init_range[1][1] + init_range[1][0]) / 2.0),
                                             (init_range[1][1] - init_range[1][0]) / 2.0, facecolor='green')
        return init
    ...
    def plot_unsafe(unsafe_range, unsafe_shape):
        if unsafe_shape == 1:  # rectangle
            unsafe = matplotlib.patches.Rectangle((unsafe_range[0][0], unsafe_range[1][0]), \
                                                  ...
                                                  unsafe_range[1][1] - unsafe_range[1][0], facecolor='red')
        elif unsafe_shape == 2:  # circle
            unsafe = matplotlib.patches.Circle(((unsafe_range[0][1] + unsafe_range[0][0]) / 2.0, \
                                                (unsafe_range[1][1] + unsafe_range[1][0]) / 2.0),
                                               (unsafe_range[1][1] - unsafe_range[1][0]) / 2.0, facecolor='red')
        else:  # a parabola?
    ...
    #     ctrl_plot_nn_input = gen_plot_data(DOMAIN, superp.PLOT_LEN_B)
    #     # apply the nn model but do not require gradient
    #     with torch.no_grad():
    #         ctrl_plot_nn_output = ctrl_nn(ctrl_plot_nn_input).reshape(superp.PLOT_LEN_B[1],
    #                                                                   superp.PLOT_LEN_B[0])  # y_size * x_size
    ...
    if len(prob.SUB_INIT) == 0:
        init = plot_init(INIT, prob.INIT_SHAPE)  # plot initial
        ax.add_patch(init)
    else:
    ...
    if len(prob.SUB_UNSAFE) == 0:
        unsafe = plot_unsafe(UNSAFE, prob.UNSAFE_SHAPE)  # plot unsafe
