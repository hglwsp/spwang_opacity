...
def cons_init(x):
    # return x[:, 0] == x[:, 0]
    inner_box = ((x[:,1] - x[:,0]) * (x[:,1] - x[:,0]) <= 0.08 ) \
                &( 0 <= x[:,0] )& (x[:,0] <= 0.05)\
                & (0.05 <= x[:,1]) & (x[:,1] < 1.03) \
                & (0 <= x[:, 3]) & (x[:, 3] <= 0) \
                & (0 <= x[:, 2]) & (x[:, 2] <= 0)
...
def cons_unsafe(x):
    inner_box = ((x[:,1] - x[:,0]) * (x[:,1] - x[:,0]) > 0.1*0.1 ) \
              & (-2 <= x[:,0]) &  (x[:,0]<= 2) \
              & (-2 <= x[:,1] ) & (x[:,1]<= 2)\
              & (-2 <= x[:,3]) & (x[:,3] <= 2)\
               & (-2 <= x[:,2]) & (x[:,2] <= 2)
...
def vector_field(x, ctrl_nn):
    ...
    def f(i, x):
        ...
        if i == 3:
            # torch.manual_seed(init_seed)
            x_ = -x[:,0] + x[:,2] + 2*x[:,0]*x[:,0] - x[:,2]*x[:,2] + u
            return x_, u  # x[:, 1] stands for x2
        elif i == 1:
            # torch.manual_seed(init_seed)
            x_ =   x[:,0] + x[:,2] + 2*x[:,0]*x[:,2] + u
            return x_, u
            # x[:, 0] stands for x1
        elif i == 4:
            # torch.manual_seed(init_seed)
            x_ = -x[:,1] + x[:,3] + 2*x[:,1]*x[:,1] - x[:,3]*x[:,3] + (ctrl_nn(controller_input))[:, 0]
            return x_, u  # x[:, 1] stands for x4
        elif i == 2:
            # torch.manual_seed(init_seed)
            x_ =   x[:,1] + x[:,3] + 2*x[:,1]*x[:,3] + (ctrl_nn(controller_input))[:, 0]
            return x_, u
            # x[:, 0] stands for x3
        else:
