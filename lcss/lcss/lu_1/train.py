...
def itr_train(barr_nn, ctrl_nn, batches_init, batches_unsafe, batches_domain,batches_goal, NUM_BATCHES):
    ...
    while num_restart < 5:
        ...
        goal_list = np.arange(NUM_BATCHES[4]) % NUM_BATCHES[3]  # D
        ...
        for epoch in range(superp.EPOCHS):  # train for a number of epochs
            ...
            np.random.shuffle(goal_list)


            # train mini-batches
            for batch_index in range(NUM_BATCHES[3]):
                # batch data selection_
                ...
                batch_goal = batches_domain[goal_list[batch_index]]
                ...
                curr_init,curr_unsafe,curr_diff,curr_u,curr_goal,curr_batch_loss= loss.calc_loss(barr_nn, ctrl_nn, batch_init, batch_unsafe,
                                                                      batch_domain, batch_goal,epoch)
