...
DIM_C = 3 # dimension of controller input
...
BLOCK_EXP_U = np.array([5,5,5]) # 0 <= BATCH_EXP <= DATA_EXP
...
BLOCK_EXP_D = np.array([5,5,5]) # 0 <= BATCH_EXP <= DATA_EXP
BLOCK_LEN_D = np.power(2, BLOCK_EXP_D) # number of batches for each dimension

DATA_EXP_G = np.array([7,7,7]) # for sampling from initial; length = prob.DIM
DATA_LEN_G = np.power(2, DATA_EXP_D) # the number of samples for each dimension of domain
BLOCK_EXP_G = np.array([5,5,5]) # 0 <= BATCH_EXP <= DATA_EXP
BLOCK_LEN_G = np.power(2, BLOCK_EXP_D) # number of batches for each dimension
############################################
# for plotting
############################################
