...
def cons_init(x):
    # return x[:, 0] == x[:, 0]
    inner_box = (x[:,0]+1)*(x[:,0]+1)+(x[:,1]+0.6)*(x[:,1]+0.6)+(x[:,2]-0.8)*(x[:,2]-0.8)<=0.01
    return inner_box



def cons_unsafe(x):
    inner_box = (x[:,0]-0.7)*(x[:,0]-0.7)+(x[:,1]+0.75)*(x[:,1]+0.75)+(x[:,2]+1)*(x[:,2]+1)<=0.04
    return inner_box

def cons_goal(x):
    inner_box = (x[:,0])*(x[:,0])+(x[:,1])*(x[:,1])+(x[:,2])*(x[:,2])<=0.05*0.05
    return inner_box
...
def vector_field(x, ctrl_nn):
    ...
    def f(i, x):
        ...
        controller_input = torch.cat((a,b,c),dim = 1)
        ...
        if i == 1:
            # torch.manual_seed(init_seed)
            x_ = -0.98*x[:,0]
            return x_  # x[:, 1] stands for x2
        elif i == 2:
            # torch.manual_seed(init_seed)
            x_ = 0.02*x[:,1]-2*x[:,1]+0.1*x[:,1]*x[:,1]*x[:,0]+x[:,2] + 0.01
            return x_
            # x[:, 0] stands for x1
        elif i == 3:
            # torch.manual_seed(init_seed)
