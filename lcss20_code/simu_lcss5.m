...
%     bc_diff(1)= 0;
    ...
    % x1(k+1) = x1(k) + 0.0001*x2(k) + u
    % x2(k+1) = -x1(k) + 0.0001*x1(k)^3 + u
    for i_time = 1: nr_time
        u1_t = (U_max(1)-U_min(1)).*rand(1) + U_min(1);
        x1_t = x1_traj(i_iter, i_time) + 0.001*v1_traj(i_iter, i_time) + u1_t;
        v1_t = 0.1837*x1_traj(i_iter, i_time) - 0.0019*(x1_traj(i_iter, i_time)-4.5)*(x1_traj(i_iter, i_time)-4.5)*(x1_traj(i_iter, i_time)-4.5)  + v1_traj(i_iter, i_time)+ u1_t ;       
...
%         end
        u2_t = ...
-0.000363564884720303*x1_traj(i_iter, i_time) - 0.000911111773164171*x2_traj(i_iter, i_time) + 0.00466314248927052*v1_traj(i_iter, i_time) - 0.00289156545156847*v2_traj(i_iter, i_time) + 0.00113935798163937*u1_t;
    %         u2_t = 0.8*x1_traj(i_iter, i_time)-0.8*x2_traj(i_iter, i_time)+1.5*v1_traj(i_iter, i_time)-1.5*v2_traj(i_iter, i_time)+u1_t;
        ...
        x2_t = x2_traj(i_iter, i_time) + 0.001*v2_traj(i_iter, i_time) + u2_t;
        v2_t = 0.1837*x2_traj(i_iter, i_time) - 0.0019*(x2_traj(i_iter, i_time)-4.5)*(x2_traj(i_iter, i_time)-4.5)*(x2_traj(i_iter, i_time)-4.5) + v2_traj(i_iter, i_time) + u2_t ;
