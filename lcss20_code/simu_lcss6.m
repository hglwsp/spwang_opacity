...
for i_iter = 1: nr_iter
    ...
    x2_0 = -2.7083*x1_0*x1_0*x1_0 + 36.4375*x1_0*x1_0 - 162 * x1_0 + 243.17;
...
%     
    ...
    for i_time = 1: nr_time
        ...
        x1_t = x1_traj(i_iter, i_time) + v1_traj(i_iter, i_time)*v1_traj(i_iter, i_time) + u1_t;
        ...
        u2_t =...
1.66072622866388*relu(-0.737115056618919*x1_traj(i_iter, i_time) - 0.283333836479221*x2_traj(i_iter, i_time) + 1.61814426461345*v1_traj(i_iter, i_time) + 0.439541151487422*v2_traj(i_iter, i_time) - 0.604675159420894*u1_t - 0.00265034358721605)...
+ 0.00345544987880607*relu(-0.306355932960885*x1_traj(i_iter, i_time) + 0.224400512116449*x2_traj(i_iter, i_time) + 0.0550078152232705*v1_traj(i_iter, i_time) - 1.24275692504471*v2_traj(i_iter, i_time) + 0.587432183800198*u1_t- 1.47797759459252)...
- 0.650916369967446*relu(-0.175483749337114*x1_traj(i_iter, i_time) - 1.38973272189551*x2_traj(i_iter, i_time) + 0.547630318624378*v1_traj(i_iter, i_time) + 1.38077303619873*v2_traj(i_iter, i_time) + 1.16468923779223*u1_t - 1.23794118287108)...
+ 0.00241234733524615*relu(-0.120314857611376*x1_traj(i_iter, i_time) + 0.339617784612894*x2_traj(i_iter, i_time) + 0.590641681021882*v1_traj(i_iter, i_time) + 0.29788976898321*v2_traj(i_iter, i_time) + 0.415729400691342*u1_t - 2.21961686192657)...
+ 0.397083986912669*relu(-0.0733115458373732*x1_traj(i_iter, i_time) - 1.42518210278342*x2_traj(i_iter, i_time) - 1.01073992744236*v1_traj(i_iter, i_time) + 0.234379949775006*v2_traj(i_iter, i_time) - 0.475416248291621*u1_t - 0.0746705033806877)...
+ 0.00335256135900997*relu(0.0441549240388186*x1_traj(i_iter, i_time) - 0.958006727893888*x2_traj(i_iter, i_time) - 0.134795208063172*v1_traj(i_iter, i_time) - 0.410284740466141*v2_traj(i_iter, i_time) + 0.247673609818938*u1_t + 0.0233264192903776)...
- 1.61726058804782*relu(0.0687460506360292*x1_traj(i_iter, i_time) - 0.225587413588054*x2_traj(i_iter, i_time) + 0.00872053679112974*v1_traj(i_iter, i_time) + 1.09781178033686*v2_traj(i_iter, i_time) - 0.414152126213077*u1_t - 1.41146179853834)...
+ 1.4415566144234*relu(0.107538461815471*x1_traj(i_iter, i_time) - 0.21891011980685*x2_traj(i_iter, i_time) - 0.196482189316215*v1_traj(i_iter, i_time) + 0.665313056579903*v2_traj(i_iter, i_time) + 2.55781143858709*u1_t - 1.46160129961965)...
- 0.0199308739479114
...
%         end
%         
        x2_t = x2_traj(i_iter, i_time) + v2_traj(i_iter, i_time)*v2_traj(i_iter, i_time) + u2_t;
...
fill([0 5.8 0],[1.2 7 7],'c','facealpha',0.2,'edgealpha',0);
fill([1.2 7 7],[0 5.8 0],'c','facealpha',0.2,'edgealpha',0);
...
plot([4 4.8 4.8 4 4 ],[4.8 4.8 6 5.2 4.8], 'LineWidth',1, 'color','k');axis([X_min(1) X_max(1) X_min(1) X_max(1)]);
axis square;
