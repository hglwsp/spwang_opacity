...
%             u2_t = u1_t - 0.001;
%         end
          u2_t = -0.0714621804683088*tanh(-0.199602761841807*x1_traj(i_iter, i_time) + 2.03938555824935*x2_traj(i_iter, i_time) + 0.458525984534168*v1_traj(i_iter, i_time) - 0.21488189742576*v2_traj(i_iter, i_time) + 0.70607889030544*u1_t)...
              - 0.0447722643882926*tanh(0.133947606963252*x1_traj(i_iter, i_time) - 2.18158809856135*x2_traj(i_iter, i_time) + 0.711033930581654*v1_traj(i_iter, i_time) - 2.36877723246828*v2_traj(i_iter, i_time) + 1.15775235451055*u1_t)...
              + 0.0791201750851987*tanh(0.170033291457262*x1_traj(i_iter, i_time) + 1.24554095928893*x2_traj(i_iter, i_time) + 1.31509364739816*v1_traj(i_iter, i_time) + 0.774037272970213*v2_traj(i_iter, i_time) + 0.08822666862857*u1_t)...
              - 0.121580902354446*tanh(0.172950064694403*x1_traj(i_iter, i_time) + 1.51216000532693*x2_traj(i_iter, i_time) + 2.40610163866335*v1_traj(i_iter, i_time) + 0.0230698079824139*v2_traj(i_iter, i_time) - 0.197349957384332*u1_t)...
              - 0.0366888195225797*tanh(0.262424196272544*x1_traj(i_iter, i_time) - 2.38001038086656*x2_traj(i_iter, i_time) + 0.586017824327192*v1_traj(i_iter, i_time) + 0.61322432937035*v2_traj(i_iter, i_time)+ 2.23121811792432*u1_t);
