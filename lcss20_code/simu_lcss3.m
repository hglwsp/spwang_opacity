...
%     bc_diff(1)= 0;
    ...
    for i_time = 1: nr_time
        ...
        x1_t = v1_traj(i_iter, i_time) + u1_t;
        v1_t = 0.01*(x1_traj(i_iter, i_time) + u1_t);
...
%         end
        ...
        u2_t = 0.0010627479849796*x1_traj(i_iter, i_time) + 0.000866147446338227*x2_traj(i_iter, i_time) + 0.00180357877584129*v1_traj(i_iter, i_time) - 0.0303569875273359*v2_traj(i_iter, i_time) - 0.0541235504854731*u1_t
%         u2_t = 0.8*x1_traj(i_iter, i_time)-0.8*x2_traj(i_iter, i_time)+1.5*v1_traj(i_iter, i_time)-1.5*v2_traj(i_iter, i_time)+u1_t;
        ...
        x2_t = v2_traj(i_iter, i_time)  + u2_t;
        v2_t = 0.01*(x2_traj(i_iter, i_time)  + u2_t);
